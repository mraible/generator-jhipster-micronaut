...
if [[ "$JHI_ENTITY" == "jdl" ]]; then
    ...
    jhipster import-jdl *.jdl --no-insight --blueprint micronaut
    ...
    #-------------------------------------------------------------------------------
    # Generate project with mhipster cli tool
    #-------------------------------------------------------------------------------
    mkdir -p "$JHI_FOLDER_APP"
    cp -f "$JHI_SAMPLES"/"$JHI_APP"/.yo-rc.json "$JHI_FOLDER_APP"/
    cd "$JHI_FOLDER_APP"
    npm link generator-jhipster-micronuat
    mhipster --force --no-insight --skip-checks --with-entities --from-cli
